import { HomeIcon, UserIcon, AcademicCapIcon, PresentationChartBarIcon } from '@heroicons/react/24/outline';
import { Link } from 'react-router-dom';

const navItems = [
  { label: 'Accueil', icon: <HomeIcon className="h-6 w-6" />, route: '/' },
  { label: 'Quiz', icon: <AcademicCapIcon className="h-6 w-6" />, route: '/history' },
  { label: 'Stats', icon: <PresentationChartBarIcon className="h-6 w-6" />, route: '/stats' },
  { label: 'Profil', icon: <UserIcon className="h-6 w-6" />, route: '/profil' },
];

export default function BottomNav() {
  return (
    <nav className="fixed bottom-0 left-0 w-full bg-white shadow flex justify-around py-2 z-50
